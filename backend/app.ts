...
// Preset beautiful high-fidelity facts list with realistic dates and images
const PRESET_GUITAR_BULLETINS = [
  {
    id: 'greeny_origin',
...
// Enable basic CORS for when this is deployed serverless on Vercel and called from GitHub Pages
app.use((req, res, next) => {
  const origin = req.headers.origin;
  // Dynamic CORS origin whitelist to allow local dev and github pages
  if (origin) {
...
// 1b. API - Dynamic daily random guitar news & facts gotten from the web
app.get('/api/guitar-bulletin', async (req, res) => {
  try {
    const apiKey = process.env.GEMINI_API_KEY;
    
    // List of local dynamic high contrast cover arts to map to bulletins safely
    const customImages = [
    ...
    if (!apiKey) {
      ...
      // Shuffle our presets randomly to keep the experience fresh on reload
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              id: { type: Type.STRING },
              category: { type: Type.STRING, description: 'Choose from: ARTIST TRIVIA, TONE SCIENCE, GEAR NEWS, GUITAR FACT, LUTHERIE TRIVIA' },
              title: { type: Type.STRING, description: 'Catchy modern headline under 60 characters' },
              description: { type: Type.STRING, description: 'Intriguing body description about the lore, fact, or news under 250 characters' },
              source: { type: Type.STRING, description: 'Journalistic publisher source like Guitar World, Premier Guitar, Vintage Guitar' },
              date: { type: Type.STRING, description: 'Relative date e.g. Today, Yesterday, Featured, Daily, Hot Gossip' }
            },
    ...
    if (textValue) {
      ...
      // Augment the Gemini response with safe local images to avoid loading broken image urls from the wild web
      const customizedBulletins = parsedBulletins.map((item: any, idx: number) => ({
        ...item,
    ...
    // Graceful fallback to our pre-compiled database on any errors (parsing, network, etc.)
    const customImages = [
      '/src/assets/images/gibson_1959_greeny_1781878999953.jpg',
      '/src/assets/images/gibson_1958_flying_v_1781879034258.jpg',
      '/src/assets/images/fender_1954_stratocaster_1781879018435.jpg',
